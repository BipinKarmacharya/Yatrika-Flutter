...
class _SettingsTabViewState extends State<SettingsTabView> {
  ...
  final interestsChanged =
      !Set.from(user.interestIds)
          .containsAll(interestProvider.selectedIds) ||
      !Set.from(interestProvider.selectedIds)
          .containsAll(user.interestIds);
...
//   _usernameController = TextEditingController(text: auth.user?.username ?? "");

//   interestProvider.load(auth.user?.interestIds ?? []);
// }
